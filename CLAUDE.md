...
- **Art/UI: scene-7 palette, PIXEL-GAME chrome, full-screen (s72 blessing as amended s74;
  ...
  Heute → Spielen hub (`features/welt/NeulandHub`, also `/welt`) is a **centered header (neutral Beta
  chip, no amber) hoisted OUTSIDE the chapter loop** (keep it there: inside the loop it duplicates the
...
- **Lese-/Hörtexte** (`src/data/texts.ts`, added s69 for redesign Phase 4.3): currently **22 `ReadingText`s / 66 checks** (2 Behörden letters, 2 workplace emails, 2 memos, workplace + daily-life announcements, voicemail scripts, plus the s75 daily-life texts and an s80 round adding a new sub-theme to each daily-life theme: an `arzt` Antibiotika-Merkblatt (announcement, arzt.behandlung), a `wohnen` Heizungs-Aushang (announcement, wohnen.probleme), a `bank` Lastschrift-Rückgabe letter (bank.zahlung), and a `bildung` Prüfungstermin voicemail (bildung.pruefung); voicemails double as TTS listening input in the 4.4 block). Schema: `id` (`tx_` prefix), `kind` (`TextKind`, closed enum: letter/email/memo/announcement/voicemail), `themeId`, `cefr`, `title`/`titleEn`, `de` (full text, blank-line paragraphs), `en` gloss, `checks` (2–3 `TextCheck` MCQs: German `question`, `options`, `answer` among options, optional English `explain`; check ids globally unique), optional `subThemeId`. All texts are authored authentic-STYLE (fictitious names/numbers), CEFR-calibrated against the CoE level descriptors (cited in provenance, never reproduced), one `text` provenance row each (`review_status: "draft"`, founder review pending). Results feed XP/theme progress, NOT vocab FSRS, so texts carry no SRS fields. When adding texts: match the schema, no em dashes, run `pnpm lint:content`.
...
- **Provenance register (`src/data/provenance.ts`, added 2026-06-15):** one `ProvenanceEntry` row per
  ...
  All 1,421 content items have provenance rows, every one carrying a non-empty `reference` (so the
  back-fill queue is empty); 1,396 are `review_status: "draft"` and 25 are `"verified"` (the founder-
...
> **Current bar (5 fixed-set slots):** `Home (fixed) · Bibliothek · Anwenden · Fortschritt · Einstellungen (fixed)`.
...
- `DEFAULT_PINNED_TABS = ["/", "/library", "/anwenden", "/analytics"]` in `nav-items.ts` (the four-zone
  nav; changed from `["/", "/vocabulary", "/quiz", "/analytics"]` in Phase 5, session 49). The store is
