...
- **Provenance register (`src/data/provenance.ts`, added 2026-06-15):** one `ProvenanceEntry` row per
  ...
  All 1,195 content items have provenance rows, every one carrying a non-empty `reference` (so the
  back-fill queue is empty); 1,170 are `review_status: "draft"` and 25 are `"verified"` (the founder-
