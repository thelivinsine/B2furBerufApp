...
- **The nav runs ONE order and setup ends in the Bibliothek** (founder s205, `PRAKTISCH-NAV.md`):
...
- **A failed cloud write is never silent** (s185). supabase-js returns `{ error }` instead of
  ...
  `useAuthStore.syncHealth` to `"failing"` ("Sync pausiert" in Settings). Never drop a Supabase result.
- **The cloud row is bounded, not append-forever** (DB audit R1/R4, s185; timers in
  `PROJECT_FOUNDATION.md`). `dailyXp`/`activeDays` keep `RETAIN_DAYS` (400) days folded into
  `activeDaysFolded`, and `pg_cron` purges abandoned guests, cache rows and learner TEXT, NULLing
  columns rather than deleting rows so limits, aggregates and the Verlauf entry survive. **A retention
  timer and the privacy copy describing it ship in the SAME change**, never the copy alone.
- **AI usage is MEASURED; only the cost is derived** (s204): every call writes an `ai_calls` row with
  the provider's token counts, priced from ONE table (`functions/_shared/aiUsage.ts`, overridable via
  `app_config.ai_rates`). Never re-hardcode a price in a function.
...
- **In-app UI language follows the LEVEL** (founder s205): A2/B1 English, B2/C1 German, and **the
