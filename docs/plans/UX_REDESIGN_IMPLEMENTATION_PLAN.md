...
**Phase 4 "The Depth" is next** (typed forward-recall, authentic Lesen/Hören); its task breakdown
was drafted in session 67 (see the Phase 4 section) and is **pending founder confirmation** of
scope/priority against the still-PROPOSED `GAME_IMPLEMENTATION_PLAN.md`. This doc merges the two
redesign reports into one executable plan:_
...
_Task breakdown drafted session 67 (2026-07-05), **DRAFT pending founder confirmation** of scope
and priority against `docs/plans/GAME_IMPLEMENTATION_PLAN.md` (still PROPOSED; its G0 prerequisite,
redesign Phases 1–3, is now fully shipped, so G1 is also eligible)._
